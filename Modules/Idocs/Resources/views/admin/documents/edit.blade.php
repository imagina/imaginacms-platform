...
@section('content-header')
  <h1>
    {{ trans('idocs::documents.title.edit document') }}
  </h1>
  <ol class="breadcrumb">
    <li><a href="{{ route('dashboard.index') }}"><i
          class="fa fa-dashboard"></i> {{ trans('core::core.breadcrumb.home') }}</a></li>
    <li><a href="{{ route('admin.idocs.document.index') }}">{{ trans('idocs::documents.title.documents') }}</a></li>
    <li class="active">{{ trans('idocs::documents.title.edit document') }}</li>
  </ol>
@stop

@section('content')
  {!! Form::open(['route' => ['admin.idocs.document.update', $document->id], 'method' => 'put']) !!}
  <div class="row">
              ...
              <div class="tab-content">
                ...
                @foreach (LaravelLocalization::getSupportedLocales() as $locale => $language)
                  ...
                  <div class="tab-pane {{ locale() == $locale ? 'active' : '' }}" id="tab_{{ $i }}">
                    @include('idocs::admin.documents.partials.edit-fields', ['lang' => $locale])
                  </div>
                @endforeach
              </div>
      ...
      <div class="row">
            ...
            <div class="box-body">
              <select class="form-control" name="category_id" id="category_id">
                @foreach ($categories as $category)
                  <option
                    value="{{$category->id}}" {{ old('category_id', $document->category_id) == $category->id ? 'selected' : '' }}> {{$category->title}}
                  </option>
                @endforeach
              </select><br>
            </div>
              ...
              <div class='form-group{{ $errors->has("status") ? ' has-error' : '' }}'>
                <label class="radio" for="inactive">
                  <input type="radio" id="status" name="status"
                         value="0" {{old('status', $document->status) == 0 ? 'checked' : '' }}>
                  {{trans('idocs::documents.status.inactive')}}
                </label>
                <label class="radio" for="active">
                  <input type="radio" id="status" name="status"
                         value="1" {{old('status', $document->status) == 1 ? 'checked' : '' }}>
            ...
            <div class="box-body">
              <select name="user_id" id="user" class="form-control">
                @foreach ($users as $user)
                  <option
                    value="{{$user->id }}" {{$user->id == old('user_id',$document->user_id) ? 'selected' : ''}}>{{$user->present()->fullname()}}
                    - ({{$user->email}})
                  </option>
                @endforeach
              </select><br>
            ...
            <div class="box box-primary">
              ...
              <div class="box-body ">
                ...
                <div class='form-group{{ $errors->has("key") ? ' has-error' : '' }}'>
                  ...
                  {!! Form::text("key", old("key",$document->key), ['class' => 'form-control', 'disable', 'placeholder' => trans('idocs::documents.form.key')]) !!}
        ...
        <div class="col-xs-12 ">
          <div class="box box-primary">
            ...
            <div class="box-body">
              <div class="tab-content">
                @mediaSingle('iconimage',$document)
              </div>
            </div>
          </div>
        </div>
