...
@section('content-header')
    <h1>
        {{ trans('ievent::categories.title.edit category') }}
    </h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('dashboard.index') }}"><i class="fa fa-dashboard"></i> {{ trans('core::core.breadcrumb.home') }}</a></li>
        <li><a href="{{ route('admin.ievent.category.index') }}">{{ trans('ievent::categories.title.categories') }}</a></li>
        <li class="active">{{ trans('ievent::categories.title.edit category') }}</li>
    </ol>
@stop

@section('content')
    {!! Form::open(['route' => ['admin.ievent.category.update', $category->id], 'method' => 'put']) !!}
    <div class="row">
        <div class="col-md-12">
                ...
                <div class="tab-content">
                    ...
                    @foreach (LaravelLocalization::getSupportedLocales() as $locale => $language)
                        ...
                        <div class="tab-pane {{ locale() == $locale ? 'active' : '' }}" id="tab_{{ $i }}">
                            @include('ievent::admin.categories.partials.edit-fields', ['lang' => $locale])
                        </div>
                    @endforeach
